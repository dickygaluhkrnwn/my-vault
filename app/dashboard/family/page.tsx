...
function FamilyContent() {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      ownerMap.forEach((group, name) => {
        ...
        group.accounts.forEach(acc => {
            ...
            else if (acc.status === 'BANNED' || acc.status === 'SUSPENDED') {
                securityPoints -= 20;
                riskyCount++;
            }
            
            if (acc.authMethod !== 'email') securityPoints += 5; 
            if (acc.category === 'FINANCE') securityPoints += 2; 
        });
        ...
        const normalizedScore = maxPossible > 0 
            ? Math.min(100, Math.max(0, Math.floor((securityPoints / maxPossible) * 100))) 
            : 0;
  ...
  return (
    <div className={`min-h-screen ${THEME.bg} text-slate-200 font-mono space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20`}>
       {/* Header */}
       <div className="flex flex-col md:flex-row md:items-center gap-4 border-b border-slate-800 pb-6">
        <div className="flex items-center gap-4">
          <div className="p-3 bg-cyan-950/30 rounded border border-cyan-500/20">
            <Shield className="text-cyan-400" size={24} />
          </div>
          <div>
            <h1 className="text-xl md:text-2xl font-bold text-white tracking-tight">ACCESS_CONTROL_MATRIX</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {members.map((member) => {
            ...
            if (member.securityScore < 50) { healthColor = "text-red-400"; HealthIcon = AlertTriangle; }
            else if (member.securityScore < 80) { healthColor = "text-yellow-400"; HealthIcon = Activity; }
            ...
            return (
              <div key={member.name} className={`group relative bg-slate-900/40 border border-slate-800 hover:border-cyan-500/30 rounded-xl overflow-hidden transition-all duration-300 hover:shadow-[0_0_20px_rgba(6,182,212,0.1)] hover:-translate-y-1 flex flex-col`}>
                ...
                {/* Header Card */}
                <div className="p-5 border-b border-slate-800/50 flex justify-between items-start bg-slate-950/50 relative overflow-hidden">
                  <div className={`absolute top-0 right-0 p-2 opacity-10 ${member.totalAccounts > 10 ? 'text-yellow-500' : 'text-slate-500'}`}>
                      <Trophy size={64} />
                  </div>
                  ...
                  <div className="flex items-center gap-3 relative z-10">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 text-[10px] text-slate-500 uppercase">
                        <Fingerprint size={10} className={healthColor} />
                        <span className={healthColor}>SCORE: {member.securityScore}/100</span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="p-5 space-y-4 flex-1">
                   <div className="space-y-2">
                     <div className="flex justify-between text-[10px] text-slate-500 font-bold uppercase tracking-wider mb-2">
                       ...
                       {member.riskyAccounts > 0 && (
                           <span className="text-red-400 flex items-center gap-1"><AlertTriangle size={10} /> {member.riskyAccounts} RISKY</span>
                       )}
                     </div>
                   ...
                   <div className="flex flex-wrap gap-2 pt-1">
                     ...
                     {Object.keys(member.categories).length > 3 && (
                         <span className="px-2 py-1 text-[10px] text-slate-600">+{Object.keys(member.categories).length - 3} more</span>
                     )}
                   </div>
                </div>
