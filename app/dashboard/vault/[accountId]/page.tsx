...
export default function AccountDetailPage({ params }: { params: Promise<{ accountId: string }> }) {
  ...
  return (
    <div className={`min-h-screen ${THEME.bg} text-slate-200 font-mono pb-20 space-y-4 md:space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500`}>
      
      {/* HEADER ACTIONS */}
      <div className="flex items-center justify-between border-b border-slate-800 pb-4">
        <button 
          onClick={() => router.back()} 
          className="flex items-center gap-2 text-slate-500 hover:text-cyan-400 transition-colors px-2 py-2 rounded group text-xs font-bold tracking-wider"
        >
          <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" />
          <span className="hidden sm:inline">{'<<'} RETURN_TO_GRID</span>
          <span className="sm:hidden">BACK</span>
        </button>
        <div className="flex gap-2">
          <button 
            onClick={() => router.push(`/dashboard/vault/edit/${account.id}`)}
            className="flex items-center gap-2 px-3 py-2 bg-cyan-950/30 text-cyan-400 border border-cyan-500/30 rounded hover:bg-cyan-500/20 hover:shadow-[0_0_15px_rgba(34,211,238,0.2)] transition-all text-xs font-bold"
          >
            <Pencil size={14} />
            <span className="hidden sm:inline">MODIFY_NODE</span>
      ...
      <div className={`rounded-xl border ${THEME.border} ${THEME.panel} overflow-hidden relative shadow-2xl`}>
        ...
        {/* Banner Header */}
        <div className="p-4 md:p-6 lg:p-8 flex flex-col md:flex-row items-start gap-4 md:gap-6 border-b border-slate-800 bg-slate-900/80">
          <div className="p-3 md:p-4 bg-slate-950 border border-slate-700 rounded-xl shadow-[0_0_20px_rgba(0,0,0,0.5)]">
            {getCategoryIcon(account.category)}
          </div>
          <div className="flex-1 min-w-0 space-y-2 w-full">
            <div>
               <div className="flex flex-col sm:flex-row sm:items-center gap-2 mb-2">
                 <h1 className="text-xl md:text-3xl font-bold text-white tracking-tight break-words uppercase">{account.serviceName}</h1>
                 {account.status === 'BANNED' && (
                    <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-red-950/50 text-red-500 border border-red-900 animate-pulse w-fit">CRITICAL_BAN</span>
                 )}
               </div>
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-slate-500 pt-2 border-t border-slate-800 mt-2 border-dashed w-full">
               <span className="flex items-center gap-1.5">
                 <User size={12} className="text-cyan-600" />
                 OWNER: <span className="text-cyan-400 font-bold">{account.owner}</span>
               </span>
               {account.linkedEmail && (
                 <span className="flex items-center gap-1.5 max-w-full">
                   <LinkIcon size={12} className="text-purple-500 shrink-0" />
                   <span className="truncate">PARENT: <span className="text-slate-300">{account.linkedEmail.split('@')[0]}</span></span>
                 </span>
               )}
            </div>
          </div>
        ...
        {/* CONTENT GRID */}
        <div className="p-4 md:p-6 lg:p-8 grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-12">
          ...
          <div className="space-y-6">
            ...
            <div className="group">
              ...
              <div className="flex items-center gap-3 p-3 bg-slate-950 rounded border border-slate-800 group-hover:border-cyan-500/50 transition-all shadow-inner">
                ...
                <span className="flex-1 font-mono text-cyan-300 break-all text-sm whitespace-normal">{account.identifier}</span>
                <button 
                  onClick={() => handleCopy(account.identifier, 'identifier')}
                  className="p-1.5 text-slate-500 hover:text-cyan-400 hover:bg-slate-800 rounded transition-colors shrink-0"
                  title="COPY_DATA"
                >
            ...
            <div className="group">
              ...
              <div className="flex items-center gap-3 p-3 bg-slate-950 rounded border border-slate-800 group-hover:border-red-500/30 transition-all shadow-inner relative overflow-hidden">
                ...
                <div className="flex-1 font-mono text-slate-300 overflow-hidden text-sm">
                  {showPassword ? (
                    <span className="break-all whitespace-normal text-red-300 animate-in fade-in">{account.password || "NULL"}</span>
                  ) : (
            ...
            {account.linkedEmail && (
              <div className="mt-4 p-3 bg-purple-950/20 rounded border border-purple-500/20 flex items-start gap-3">
                <Share2 size={16} className="text-purple-400 mt-0.5 shrink-0" />
                <div className="min-w-0">
                  <h4 className="text-[10px] font-bold text-purple-300 uppercase tracking-wide mb-1">Parent_Node_Link</h4>
                  <p className="text-xs text-purple-200/70 break-all font-mono whitespace-normal">{account.linkedEmail}</p>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              {renderSpecificDetails(account, copiedField, handleCopy)}
        ...
        {connectedAccounts.length > 0 && (
          <div className="border-t border-slate-800 bg-slate-950/30 p-4 md:p-6 lg:p-8">
        ...
        {/* FOOTER METADATA */}
        <div className="bg-slate-950 px-4 md:px-6 py-3 border-t border-slate-800 flex flex-col sm:flex-row justify-between text-[10px] text-slate-600 font-mono gap-2">
          ...
          <div className="truncate">ID: <span className="text-slate-500">{account.id}</span></div>
        </div>
      </div>
    </div>
  );
...
function renderSpecificDetails(account: Account, copiedField: string | null, handleCopy: any) {
    ...
    if (account.category === "UTILITY") {
        return (
            <>
                ...
                {d.phoneLinked && <DetailRow label="RECOVERY_COMMS" value={d.phoneLinked} icon={<Smartphone size={14}/>} onCopy={() => handleCopy(d.phoneLinked, 'phone')} copied={copiedField === 'phone'} />}
            </>
        );
    }
    if (account.category === "EDUCATION") {
        return (
            <>
                {d.institution && <DetailRow label="INSTITUTION" value={d.institution} icon={<GraduationCap size={14}/>} />}
                {d.course && <DetailRow label="COURSE_MODULE" value={d.course} icon={<BookOpen size={14}/>} />}
                {d.level && <DetailRow label="CURRENT_LEVEL" value={d.level} icon={<Award size={14}/>} />}
                {d.progress && <DetailRow label="PROGRESS_PCT" value={d.progress} icon={<Percent size={14}/>} />}
            </>
        );
    }
    if (account.category === "ECOMMERCE") {
        return (
            <>
                {d.shop_name && <DetailRow label="SHOP_IDENTIFIER" value={d.shop_name} icon={<ShoppingBag size={14}/>} />}
                {d.paylater_limit && <DetailRow label="PAYLATER_CAP" value={d.paylater_limit} icon={<CreditCard size={14}/>} />}
                {d.phone_linked && <DetailRow label="LINKED_MOBILE" value={d.phone_linked} icon={<Smartphone size={14}/>} />}
            </>
        );
    }
    if (account.category === "GAME") {
        return (
            <>
                {d.ign && <DetailRow label="IGN_HANDLE" value={d.ign} icon={<User size={14}/>} onCopy={() => handleCopy(d.ign, 'ign')} copied={copiedField === 'ign'} />}
                {d.server && <DetailRow label="SERVER_REGION" value={d.server} icon={<Globe size={14}/>} />}
                {d.rank && <DetailRow label="COMPETITIVE_RANK" value={d.rank} icon={<Activity size={14}/>} />}
                {d.level && <DetailRow label="LEVEL_IND" value={d.level} icon={<Activity size={14}/>} />}
            </>
        );
    }
    if (account.category === "FINANCE") {
        return (
            <>
                {d.accountNumber && <DetailRow label="ACC_NUMBER" value={d.accountNumber} icon={<CreditCard size={14}/>} onCopy={() => handleCopy(d.accountNumber, 'norek')} copied={copiedField === 'norek'} />}
                {d.pinAtm && <DetailRow label="ATM_PIN_CODE" value={d.pinAtm} icon={<Hash size={14}/>} isSecret />}
                {d.pinApp && <DetailRow label="APP_ACCESS_PIN" value={d.pinApp} icon={<Smartphone size={14}/>} isSecret />}
            </>
        );
    }
            ...
            <>
                {d.profileUrl && (
                    <div className="flex flex-col py-2 border-b border-slate-800/50 last:border-0 hover:bg-white/5 px-2 -mx-2 rounded transition-colors">
                        ...
                        <a href={d.profileUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 mt-1 text-blue-400 hover:text-blue-300 text-xs font-mono break-all whitespace-normal">
                            <ExternalLink size={12} className="shrink-0" />
                            {d.profileUrl}
                        </a>
                    </div>
                )}
                {d.phoneLinked && <DetailRow label="LINKED_MOBILE" value={d.phoneLinked} icon={<Smartphone size={14}/>} onCopy={() => handleCopy(d.phoneLinked, 'phone')} copied={copiedField === 'phone'} />}
            </>
        );
    }
    return null;
...
function DetailRow({ label, value, icon, onCopy, copied, isSecret }: any) {
  ...
  return (
    <div className="flex flex-col py-2 border-b border-slate-800/50 last:border-0 hover:bg-white/5 px-2 -mx-2 rounded transition-colors group/row">
      <span className="text-[9px] text-slate-500 uppercase tracking-wider">{label}</span>
      <div className="flex items-center gap-2 mt-1">
        <span className="text-slate-600 shrink-0">{icon}</span>
        <span className="font-mono text-slate-300 flex-1 text-xs break-all whitespace-pre-wrap">
          {isSecret && !show ? "********" : value}
        </span>
        
        {isSecret && (
          <button onClick={() => setShow(!show)} className="text-slate-600 hover:text-slate-300 px-1 shrink-0">
            {show ? <EyeOff size={12}/> : <Eye size={12}/>}
          </button>
        )}
        
        {onCopy && (
          <button onClick={onCopy} className="text-slate-600 hover:text-cyan-400 px-1 opacity-0 group-hover/row:opacity-100 transition-opacity shrink-0" title="COPY">
