...
import { 
  Save, ArrowLeft, Gamepad2, Wallet, Share2, Briefcase, Mail, Music, 
...
export default function CreateAccountPage() {
  ...
  const [formData, setFormData] = useState({
    serviceName: "",
    websiteUrl: "", // BARU: Untuk fetch favicon
  ...
  // Fetch All Accounts for Suggestions (SECURE QUERY)
  useEffect(() => {
  ...
  // Rekomendasi Auth Method berdasarkan Kategori
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg("Sistem gagal menyimpan data. Silakan coba lagi.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const textDict = {
    formal: {
      headerTitle: "Tambah Kredensial Baru",
      headerSub: "Simpan informasi login dengan aman ke dalam vault.",
    ...
    casual: {
      headerTitle: "Tambah Akun Baru",
      headerSub: "Simpan info login kamu dengan aman di brankas.",
    ...
    hacker: {
      headerTitle: "SMART_DATA_INJECTION",
      headerSub: "ESTABLISHING NEW SECURE RECORD...",
  ...
  return (
    <div className={cn("w-full max-w-6xl mx-auto pb-24 space-y-6 lg:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500", cs.wrapper)}>
      ...
      {/* HEADER */}
      <div className="flex items-center gap-4 border-b border-inherit pb-6 pt-2">
        <button 
          type="button"
          onClick={() => router.back()} 
          className={cn("p-2.5 transition-colors group", cs.btnOutline)}
        >
          <ArrowLeft size={20} className={cn("transition-colors", cs.textSub, "group-hover:text-blue-500", theme === 'hacker' && 'group-hover:text-green-400', theme === 'casual' && 'group-hover:text-orange-500')} />
        </button>
        <div>
          ...
          <p className={cn("text-xs tracking-widest mt-1", cs.textSub, theme === 'hacker' && 'font-mono uppercase')}>
            {t.headerSub}
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="flex flex-col sm:flex-row gap-4 pt-8 border-t border-slate-200 dark:border-slate-800 -mx-4 md:mx-0 px-4 md:px-0">
          ...
          <button type="submit" disabled={loading} className={cn("flex-[2] px-5 py-4 text-sm font-bold tracking-widest uppercase transition-all shadow-md flex justify-center items-center gap-2", cs.btnPrimary, loading && 'opacity-70 cursor-not-allowed')}>
            {loading ? <Loader2 className="animate-spin" size={18} /> : <Save size={18} />} {t.btnSave}
          </button>
        </div>
      </form>
    </div>
